...
import { HexahackStatsPanel, HexaskyStatsPanel, HexaflowStatsPanel, HexaecoStatsPanel, StatsPanel, StatsTabs, type StatsGame } from "../stats/StatsView";
...
export function PublicProfileView({ profile, onBack }: { profile: PublicPlayerProfileDto; onBack: () => void }) {
  ...
  const stats = { overall: profile.overallStats, hexaword: profile.hexawordStats, hexahack: profile.hexahackStats, hexasky: profile.hexaskyStats, hexaflow: profile.hexaflowStats, hexastar: profile.hexastarStats, hexaeco: profile.hexaecoStats };
  return (
    <section className="profile-view public-profile-view" aria-label={`Profilo di ${profile.nickname}`}>
      ...
      <div className="profile-stats" aria-label="Statistiche pubbliche">
        ...
        {activeStats === "hexahack" ? <HexahackStatsPanel stats={stats.hexahack} /> : activeStats === "hexasky" ? <HexaskyStatsPanel stats={stats.hexasky} /> : activeStats === "hexaflow" ? <HexaflowStatsPanel stats={stats.hexaflow}/> : activeStats === "hexaeco" ? <HexaecoStatsPanel stats={stats.hexaeco}/> : <StatsPanel stats={stats[activeStats]} />}
      </div>
    </section>
  );
}
